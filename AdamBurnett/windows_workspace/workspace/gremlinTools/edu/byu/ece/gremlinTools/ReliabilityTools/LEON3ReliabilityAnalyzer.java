...
public class LEON3ReliabilityAnalyzer extends ReliabilityAnalyzer {
	
	
	public LEON3ReliabilityAnalyzer(String xdlFilename, String vhdlFilename, String farFileName, int type) {
		super(xdlFilename);
		
		_xdlFile = xdlFilename;
		_vhdFile = vhdlFilename;
		_farFile = farFileName;
		_type = type;
		_performancePenalty = 1.0;
		
		switch (type) {
			case LEON3Result.SW:
				_performancePenalty = 1.64;
				break;
			case LEON3Result.DWC:
				_performancePenalty = 1.01;
				break;
			case LEON3Result.TMR:
				_performancePenalty = 1.00;
				break;
			default:
				break;				
		}
		
		_parse = new ParseSYNvhdl(_vhdFile, _type);
		_parseXDL();
		_resetSignalComponentHash();
		_groupTypes();
		_determineComponentArea();
		
		_numBitsConsidered = _parseXRTCfile();
		_bitsNotAnalized = AnalyzeBits(_xrtcHash);
		_fillClassTypes();
//		_splitPipeTypes();
		_analyze();
	}
	...
	public void writeSummary(PrintWriter pw) {
		...
		if (_type != LEON3Result.ORIG) {
			...
			pw.printf(" \t (%3.3f)", dloopP);
			pw.append("\tLATENCY: "+_errLatencyHash.get(LEON3Result.DLOOP)+"\n");
		}
	...
	public void writeArea(PrintWriter pw) {
		...
		pw.append(String.format("%1$12s", "rMTTFsdc"));
		pw.append(String.format("%1$12s", "rMTTFdue"));
...
//				double rmttf = (avf == 0) ? 0 : 100 * factor2 * (1 / avf);
				...
				double rmttfsdc = (sdc == 0) ? 0 : 1000 * 1000 * (1 / (double)sdc);
				double rmttfdue = (due == 0) ? 0 : 1000 * 1000 * (1 / (double)due);
				...
				pw.append(String.format("%1$12.2f", rmttfsdc));
				pw.append(String.format("%1$12.2f", rmttfdue));
...
//		double rmttf = (avf == 0) ? 0 : 100 * (1 / avf);
		...
		double mitf = (ace == 0) ? 0 : 1000 * 1000 * (_IPC / (ace * _performancePenalty));
		double rmttfsdc = (totSDC == 0) ? 0 : (double)((double)1000000 / (double)totSDC);
		double rmttfdue = (totDUE == 0) ? 0 : (double)((double)1000000 / (double)totDUE);
		double rmttf = (ace == 0) ? 0 : 1000 * 1000 * (1 / ace);
    ...
    public static void main(String args []) {
    	_parseArgs(args);
    	LEON3ReliabilityAnalyzer analyzer = new LEON3ReliabilityAnalyzer(_xdlFile, _vhdFile, _farFile, _type);        
...
//			if (result.getTop() > 0) {
				if (!(_notrap && result.isTrapType()) && !(!_doType2 && result.getType() == 2)) {
...
//		_detectXRTCcnt += _errCntHash.get(LEON3Result.ERR);
	...
	protected void _parseXDL() {
		...
		VHDLComponent clkComp = typeComponentMap.get(ParseSYNvhdl.CLK);
		VHDLComponent rstComp = typeComponentMap.get(ParseSYNvhdl.RST);
		VHDLComponent topComp = typeComponentMap.get(ParseSYNvhdl.RST);
		...
		for (Net net : design.getNets()) {
			...
			VHDLComponent comp = signalComponentMap.get(netName);
			if (clkComp.isSignalInFilterList(netName)) {
				found = true;
				if (comp != null && !comp.getName().equalsIgnoreCase(ParseSYNvhdl.CLK)) {
					VHDLSignal vsiSignal = comp.removeSignal(netName);
					clkComp.addSignal(vsiSignal);
				} else if (comp == null) {
					VHDLSignal vsignal = new VHDLSignal(ParseSYNvhdl.UNKNOWN, netName);
					clkComp.addSignal(vsignal);					
				}
				clkComp.addNet(netName, net);
				System.out.println("\tCLK FOUND: "+clkComp.getName());				
			} else if ((netName.contains("rst_") || netName.endsWith("rst") || netName.contains("rstn") || 
					netName.contains("qrst") || netName.contains("sysrst")) && 
						!netName.endsWith("burst") && !netName.contains("rst_cnt")) {
				found = true;
				if (comp != null && !comp.getName().equalsIgnoreCase(netName)) {
					VHDLSignal vsignal = comp.removeSignal(netName);
					rstComp.addSignal(vsignal);
				} else if (comp == null) {
					VHDLSignal vsignal = new VHDLSignal(ParseSYNvhdl.UNKNOWN, netName);
					rstComp.addSignal(vsignal);					
				}
				rstComp.addNet(netName, net);
				System.out.println("\tRST FOUND: "+rstComp.getName());				
			} else if (topComp.isSignalInFilterList(netName)) {
				found = true;
				if (comp != null && !comp.getName().equalsIgnoreCase(netName)) {
					VHDLSignal vsignal = comp.removeSignal(netName);
					topComp.addSignal(vsignal);
				} else if (comp == null) {
					VHDLSignal vsignal = new VHDLSignal(ParseSYNvhdl.UNKNOWN, netName);
					topComp.addSignal(vsignal);					
				}
				topComp.addNet(netName, net);
				System.out.println("\tTOP FOUND: "+topComp.getName());							
			} else if (comp == null) {				
				for (String type : ParseSYNvhdl.CLASSESLIST) {
					VHDLComponent typeComp = typeComponentMap.get(type);
					if (typeComp != null) {
						if (typeComp.isSignalInFilterList(netName)) {
							found = true;
							VHDLSignal vsignal = new VHDLSignal(ParseSYNvhdl.UNKNOWN, netName);
							typeComp.addSignal(vsignal);
							typeComp.addNet(netName, net);
							System.out.println("\tNULL FOUND: "+typeComp.getName());
							break;
						}
					}
				}
				...
				found = true;
				comp.addNet(netName, net);
				System.out.println("\tTRUE FOUND: "+comp.getName());
			}
			
			if (!found) {
				System.out.println("UNKNOWN XDL NET: "+netName);
				VHDLSignal vsignal = new VHDLSignal(ParseSYNvhdl.UNKNOWN, netName);
				typeComponentMap.get(ParseSYNvhdl.UNKNOWN).addSignal(vsignal);
				typeComponentMap.get(ParseSYNvhdl.UNKNOWN).addNet(netName, net);
		...
		for (Instance inst : design.getInstances()) {
			...
			VHDLComponent comp = signalComponentMap.get(instName);
			if (clkComp.isSignalInFilterList(instName)) {
				found = true;
				if (comp != null && !comp.getName().equalsIgnoreCase(ParseSYNvhdl.CLK)) {
					VHDLSignal vsignal = comp.removeSignal(instName);
					clkComp.addSignal(vsignal);
				} else if (comp == null) {
					VHDLSignal vsignal = new VHDLSignal(ParseSYNvhdl.UNKNOWN, instName);
					clkComp.addSignal(vsignal);
				} 
				clkComp.addInstance(instName, inst);
				System.out.println("\tCLK FOUND: "+clkComp.getName());				
			} else if ((instName.contains("rst_") || instName.endsWith("rst") || instName.contains("rstn")) && 
						!instName.endsWith("burst") && !instName.contains("rst_cnt")) {
				found = true;
				if (comp != null && !comp.getName().equalsIgnoreCase(ParseSYNvhdl.RST)) {
					VHDLSignal vsignal = comp.removeSignal(instName);
					rstComp.addSignal(vsignal);
				} else if (comp == null) {
					VHDLSignal vsignal = new VHDLSignal(ParseSYNvhdl.UNKNOWN, instName);
				...
				rstComp.addInstance(instName, inst);
				System.out.println("\tRST FOUND: "+rstComp.getName());				
			} else if (topComp.isSignalInFilterList(instName)) {
				found = true;
				if (comp != null && !comp.getName().equalsIgnoreCase(ParseSYNvhdl.TOP)) {
					VHDLSignal vsignal = comp.removeSignal(instName);
					topComp.addSignal(vsignal);
				} else if (comp == null) {
					VHDLSignal vsignal = new VHDLSignal(ParseSYNvhdl.UNKNOWN, instName);
				...
				topComp.addInstance(instName, inst);
				System.out.println("\tTOP FOUND: "+topComp.getName());							
			} else if (comp == null) {
				for (String type : ParseSYNvhdl.CLASSESLIST) {
					VHDLComponent typeComp = typeComponentMap.get(type);
					if (typeComp != null) {
						if (typeComp.isSignalInFilterList(instName)) {
							found = true;
							VHDLSignal vsignal = new VHDLSignal(ParseSYNvhdl.UNKNOWN, instName);
							typeComp.addSignal(vsignal);
							typeComp.addInstance(instName, inst);
							System.out.println("\tFOUND: "+typeComp.getName());
							break;
						}
					}
				}
				...
				found = true;
				comp.addInstance(instName, inst);
				System.out.println("\tTRUE FOUND: "+comp.getName());
			}
			
			if (!found) {
				System.out.println("UNKNOWN XDL INST: "+instName);
				VHDLSignal vsignal = new VHDLSignal(ParseSYNvhdl.UNKNOWN, instName);
				typeComponentMap.get(ParseSYNvhdl.UNKNOWN).addSignal(vsignal);
				typeComponentMap.get(ParseSYNvhdl.UNKNOWN).addInstance(instName, inst);
			}
		}
	}
...
//    	_xrtcHash = new HashMap<LEON3Result, V4ConfigurationBit>();
	...
	protected static void _parseArgs(String[] args) {
		...
		for (int i = 3; i < args.length; i++) {
			if (args[i].equals("-h") || args[i].equals("-help")) {
				...
				_joshOutput = true;
			} else if (args[i].equals("-no2")) {
				_doType2 = false;
			} else if (args[i].equals("-notrap")) {
				_notrap = true;;
	...
	protected static String _usage() {
		...
		sb.append("\t-no2 \t\t Don't parse FARs of type 2 (BRAM DATA).\n");
		sb.append("\t-notrap \t\t Don't parse FARs that are trap detected.\n");
...
//    protected HashMap<LEON3Result, V4ConfigurationBit> _xrtcHash;
    ...
    protected ArrayList<LEON3Result> _bitsNotAnalized;
    ...
    protected static boolean _doType2 = true;
    protected static boolean _notrap = false;
