...
begin
  --slv_reg0 is being used as a place to store the count (lower 32 bits),
  --slv_reg1 is storing the upper 32 bits of the counter
  ...
  begin
	if(en = '0') then -- Basic reset if no enable.
		count <= (others=>'0');
	elsif (ring_osc'event and ring_osc = '1') then
		count <= std_logic_vector(unsigned(count) + 1);
		slv_reg0 <= count(31 downto 0);
		slv_reg1 <= count(63 downto 32);
	end if;
  end process;
